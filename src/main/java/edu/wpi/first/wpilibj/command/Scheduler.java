...
public final class Scheduler {
  ...
  private void _add(Command command) {
    ...
    if (!commandTableContainsKey(command)) {
  ...
  private boolean commandTableContainsKey(Object key) {
    Enumeration keys = m_commandTable.keys();
  ...
  private void commandTableRemove(Object key) {
    Hashtable<Command, LinkedListElement> newCommandTable = new Hashtable<>();
    Enumeration keys = m_commandTable.keys();
    while (keys.hasMoreElements()) {
      Command nextKey = (Command)keys.nextElement();
      if (key != nextKey) {
        newCommandTable.put(nextKey, m_commandTable.get(nextKey));
      }
    }
    m_commandTable = newCommandTable;
  }
  ...
  void remove(Command command) {
    if (command == null || !commandTableContainsKey(command)) {
      return;
    }
    LinkedListElement element = m_commandTable.get(command);
    commandTableRemove(command);
